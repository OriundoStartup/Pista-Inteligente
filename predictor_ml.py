...
class PredictorHipico:
    ...
    def generar_predicciones_jornada(self):
        ...
        print(f"\n🔮 Generando predicciones para {len(df_programa)} carreras...")
        ...
        for idx, row in df_programa.iterrows():
            ...
            caballos_str = row['caballos']
            
            # Parsear caballos
            try:
                caballos = [int(c.strip()) for c in caballos_str.split(',') if c.strip().isdigit()]
            except:
                continue
